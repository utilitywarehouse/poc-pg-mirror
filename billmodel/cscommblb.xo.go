...
// Exists determines if the CscommBlb exists in the database.
func (cb *CscommBlb) Exists() bool {
	return cb._exists
}

// Deleted provides information if the CscommBlb has been deleted from the database.
func (cb *CscommBlb) Deleted() bool {
	return cb._deleted
}

// Insert inserts the CscommBlb to the database.
func (cb *CscommBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.cscomm_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the CscommBlb in the database.
func (cb *CscommBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.cscomm_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the CscommBlb to the database.
func (cb *CscommBlb) Save(db XODB) error {
	if cb.Exists() {
		return cb.Update(db)
	}
...
// Upsert performs an upsert for CscommBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (cb *CscommBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the CscommBlb from the database.
func (cb *CscommBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.cscomm_blb WHERE blb_lrn = $1`
