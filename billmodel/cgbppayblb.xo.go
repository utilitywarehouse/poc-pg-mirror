...
// Exists determines if the CgbppayBlb exists in the database.
func (cb *CgbppayBlb) Exists() bool {
	return cb._exists
}

// Deleted provides information if the CgbppayBlb has been deleted from the database.
func (cb *CgbppayBlb) Deleted() bool {
	return cb._deleted
}

// Insert inserts the CgbppayBlb to the database.
func (cb *CgbppayBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.cgbppay_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the CgbppayBlb in the database.
func (cb *CgbppayBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.cgbppay_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the CgbppayBlb to the database.
func (cb *CgbppayBlb) Save(db XODB) error {
	if cb.Exists() {
		return cb.Update(db)
	}
...
// Upsert performs an upsert for CgbppayBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (cb *CgbppayBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the CgbppayBlb from the database.
func (cb *CgbppayBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.cgbppay_blb WHERE blb_lrn = $1`
