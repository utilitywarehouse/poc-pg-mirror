...
// Exists determines if the StockBlb exists in the database.
func (sb *StockBlb) Exists() bool {
	return sb._exists
}

// Deleted provides information if the StockBlb has been deleted from the database.
func (sb *StockBlb) Deleted() bool {
	return sb._deleted
}

// Insert inserts the StockBlb to the database.
func (sb *StockBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.stock_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the StockBlb in the database.
func (sb *StockBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.stock_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the StockBlb to the database.
func (sb *StockBlb) Save(db XODB) error {
	if sb.Exists() {
		return sb.Update(db)
	}
...
// Upsert performs an upsert for StockBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (sb *StockBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the StockBlb from the database.
func (sb *StockBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.stock_blb WHERE blb_lrn = $1`
