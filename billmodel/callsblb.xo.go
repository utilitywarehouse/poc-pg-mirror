...
// Exists determines if the CallsBlb exists in the database.
func (cb *CallsBlb) Exists() bool {
	return cb._exists
}

// Deleted provides information if the CallsBlb has been deleted from the database.
func (cb *CallsBlb) Deleted() bool {
	return cb._deleted
}

// Insert inserts the CallsBlb to the database.
func (cb *CallsBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.calls_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the CallsBlb in the database.
func (cb *CallsBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.calls_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the CallsBlb to the database.
func (cb *CallsBlb) Save(db XODB) error {
	if cb.Exists() {
		return cb.Update(db)
	}
...
// Upsert performs an upsert for CallsBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (cb *CallsBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the CallsBlb from the database.
func (cb *CallsBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.calls_blb WHERE blb_lrn = $1`
