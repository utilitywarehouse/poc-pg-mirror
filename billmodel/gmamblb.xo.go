...
// Exists determines if the GmamBlb exists in the database.
func (gb *GmamBlb) Exists() bool {
	return gb._exists
}

// Deleted provides information if the GmamBlb has been deleted from the database.
func (gb *GmamBlb) Deleted() bool {
	return gb._deleted
}

// Insert inserts the GmamBlb to the database.
func (gb *GmamBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.gmam_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the GmamBlb in the database.
func (gb *GmamBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.gmam_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the GmamBlb to the database.
func (gb *GmamBlb) Save(db XODB) error {
	if gb.Exists() {
		return gb.Update(db)
	}
...
// Upsert performs an upsert for GmamBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (gb *GmamBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the GmamBlb from the database.
func (gb *GmamBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.gmam_blb WHERE blb_lrn = $1`
