...
// Exists determines if the LedgerpBlb exists in the database.
func (lb *LedgerpBlb) Exists() bool {
	return lb._exists
}

// Deleted provides information if the LedgerpBlb has been deleted from the database.
func (lb *LedgerpBlb) Deleted() bool {
	return lb._deleted
}

// Insert inserts the LedgerpBlb to the database.
func (lb *LedgerpBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.ledgerp_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the LedgerpBlb in the database.
func (lb *LedgerpBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.ledgerp_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the LedgerpBlb to the database.
func (lb *LedgerpBlb) Save(db XODB) error {
	if lb.Exists() {
		return lb.Update(db)
	}
...
// Upsert performs an upsert for LedgerpBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (lb *LedgerpBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the LedgerpBlb from the database.
func (lb *LedgerpBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.ledgerp_blb WHERE blb_lrn = $1`
