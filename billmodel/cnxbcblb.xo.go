...
// Exists determines if the CnxbcBlb exists in the database.
func (cb *CnxbcBlb) Exists() bool {
	return cb._exists
}

// Deleted provides information if the CnxbcBlb has been deleted from the database.
func (cb *CnxbcBlb) Deleted() bool {
	return cb._deleted
}

// Insert inserts the CnxbcBlb to the database.
func (cb *CnxbcBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.cnxbc_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the CnxbcBlb in the database.
func (cb *CnxbcBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.cnxbc_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the CnxbcBlb to the database.
func (cb *CnxbcBlb) Save(db XODB) error {
	if cb.Exists() {
		return cb.Update(db)
	}
...
// Upsert performs an upsert for CnxbcBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (cb *CnxbcBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the CnxbcBlb from the database.
func (cb *CnxbcBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.cnxbc_blb WHERE blb_lrn = $1`
