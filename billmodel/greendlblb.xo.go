...
// Exists determines if the GreendlBlb exists in the database.
func (gb *GreendlBlb) Exists() bool {
	return gb._exists
}

// Deleted provides information if the GreendlBlb has been deleted from the database.
func (gb *GreendlBlb) Deleted() bool {
	return gb._deleted
}

// Insert inserts the GreendlBlb to the database.
func (gb *GreendlBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.greendl_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the GreendlBlb in the database.
func (gb *GreendlBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.greendl_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the GreendlBlb to the database.
func (gb *GreendlBlb) Save(db XODB) error {
	if gb.Exists() {
		return gb.Update(db)
	}
...
// Upsert performs an upsert for GreendlBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (gb *GreendlBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the GreendlBlb from the database.
func (gb *GreendlBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.greendl_blb WHERE blb_lrn = $1`
