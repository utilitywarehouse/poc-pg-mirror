...
// Exists determines if the PledgerBlb exists in the database.
func (pb *PledgerBlb) Exists() bool {
	return pb._exists
}

// Deleted provides information if the PledgerBlb has been deleted from the database.
func (pb *PledgerBlb) Deleted() bool {
	return pb._deleted
}

// Insert inserts the PledgerBlb to the database.
func (pb *PledgerBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.pledger_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the PledgerBlb in the database.
func (pb *PledgerBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.pledger_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the PledgerBlb to the database.
func (pb *PledgerBlb) Save(db XODB) error {
	if pb.Exists() {
		return pb.Update(db)
	}
...
// Upsert performs an upsert for PledgerBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (pb *PledgerBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the PledgerBlb from the database.
func (pb *PledgerBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.pledger_blb WHERE blb_lrn = $1`
