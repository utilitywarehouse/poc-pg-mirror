...
// Exists determines if the PperrorBlb exists in the database.
func (pb *PperrorBlb) Exists() bool {
	return pb._exists
}

// Deleted provides information if the PperrorBlb has been deleted from the database.
func (pb *PperrorBlb) Deleted() bool {
	return pb._deleted
}

// Insert inserts the PperrorBlb to the database.
func (pb *PperrorBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.pperror_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the PperrorBlb in the database.
func (pb *PperrorBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.pperror_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the PperrorBlb to the database.
func (pb *PperrorBlb) Save(db XODB) error {
	if pb.Exists() {
		return pb.Update(db)
	}
...
// Upsert performs an upsert for PperrorBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (pb *PperrorBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the PperrorBlb from the database.
func (pb *PperrorBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.pperror_blb WHERE blb_lrn = $1`
