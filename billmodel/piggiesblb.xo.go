...
// Exists determines if the PiggiesBlb exists in the database.
func (pb *PiggiesBlb) Exists() bool {
	return pb._exists
}

// Deleted provides information if the PiggiesBlb has been deleted from the database.
func (pb *PiggiesBlb) Deleted() bool {
	return pb._deleted
}

// Insert inserts the PiggiesBlb to the database.
func (pb *PiggiesBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.piggies_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the PiggiesBlb in the database.
func (pb *PiggiesBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.piggies_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the PiggiesBlb to the database.
func (pb *PiggiesBlb) Save(db XODB) error {
	if pb.Exists() {
		return pb.Update(db)
	}
...
// Upsert performs an upsert for PiggiesBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (pb *PiggiesBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the PiggiesBlb from the database.
func (pb *PiggiesBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.piggies_blb WHERE blb_lrn = $1`
