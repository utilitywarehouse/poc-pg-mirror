...
// Exists determines if the CtfixedBlb exists in the database.
func (cb *CtfixedBlb) Exists() bool {
	return cb._exists
}

// Deleted provides information if the CtfixedBlb has been deleted from the database.
func (cb *CtfixedBlb) Deleted() bool {
	return cb._deleted
}

// Insert inserts the CtfixedBlb to the database.
func (cb *CtfixedBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.ctfixed_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the CtfixedBlb in the database.
func (cb *CtfixedBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.ctfixed_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the CtfixedBlb to the database.
func (cb *CtfixedBlb) Save(db XODB) error {
	if cb.Exists() {
		return cb.Update(db)
	}
...
// Upsert performs an upsert for CtfixedBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (cb *CtfixedBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the CtfixedBlb from the database.
func (cb *CtfixedBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.ctfixed_blb WHERE blb_lrn = $1`
