...
// Exists determines if the EcurveBlb exists in the database.
func (eb *EcurveBlb) Exists() bool {
	return eb._exists
}

// Deleted provides information if the EcurveBlb has been deleted from the database.
func (eb *EcurveBlb) Deleted() bool {
	return eb._deleted
}

// Insert inserts the EcurveBlb to the database.
func (eb *EcurveBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.ecurve_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the EcurveBlb in the database.
func (eb *EcurveBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.ecurve_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the EcurveBlb to the database.
func (eb *EcurveBlb) Save(db XODB) error {
	if eb.Exists() {
		return eb.Update(db)
	}
...
// Upsert performs an upsert for EcurveBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (eb *EcurveBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the EcurveBlb from the database.
func (eb *EcurveBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.ecurve_blb WHERE blb_lrn = $1`
