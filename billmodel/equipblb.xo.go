...
// Exists determines if the EquipBlb exists in the database.
func (eb *EquipBlb) Exists() bool {
	return eb._exists
}

// Deleted provides information if the EquipBlb has been deleted from the database.
func (eb *EquipBlb) Deleted() bool {
	return eb._deleted
}

// Insert inserts the EquipBlb to the database.
func (eb *EquipBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.equip_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the EquipBlb in the database.
func (eb *EquipBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.equip_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the EquipBlb to the database.
func (eb *EquipBlb) Save(db XODB) error {
	if eb.Exists() {
		return eb.Update(db)
	}
...
// Upsert performs an upsert for EquipBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (eb *EquipBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the EquipBlb from the database.
func (eb *EquipBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.equip_blb WHERE blb_lrn = $1`
