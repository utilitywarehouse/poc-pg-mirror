...
// Exists determines if the PropvalBlb exists in the database.
func (pb *PropvalBlb) Exists() bool {
	return pb._exists
}

// Deleted provides information if the PropvalBlb has been deleted from the database.
func (pb *PropvalBlb) Deleted() bool {
	return pb._deleted
}

// Insert inserts the PropvalBlb to the database.
func (pb *PropvalBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.propval_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the PropvalBlb in the database.
func (pb *PropvalBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.propval_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the PropvalBlb to the database.
func (pb *PropvalBlb) Save(db XODB) error {
	if pb.Exists() {
		return pb.Update(db)
	}
...
// Upsert performs an upsert for PropvalBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (pb *PropvalBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the PropvalBlb from the database.
func (pb *PropvalBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.propval_blb WHERE blb_lrn = $1`
