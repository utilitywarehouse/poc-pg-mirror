...
// Exists determines if the PortingBlb exists in the database.
func (pb *PortingBlb) Exists() bool {
	return pb._exists
}

// Deleted provides information if the PortingBlb has been deleted from the database.
func (pb *PortingBlb) Deleted() bool {
	return pb._deleted
}

// Insert inserts the PortingBlb to the database.
func (pb *PortingBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.porting_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the PortingBlb in the database.
func (pb *PortingBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.porting_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the PortingBlb to the database.
func (pb *PortingBlb) Save(db XODB) error {
	if pb.Exists() {
		return pb.Update(db)
	}
...
// Upsert performs an upsert for PortingBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (pb *PortingBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the PortingBlb from the database.
func (pb *PortingBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.porting_blb WHERE blb_lrn = $1`
