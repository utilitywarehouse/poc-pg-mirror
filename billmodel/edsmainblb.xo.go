...
// Exists determines if the EdsmainBlb exists in the database.
func (eb *EdsmainBlb) Exists() bool {
	return eb._exists
}

// Deleted provides information if the EdsmainBlb has been deleted from the database.
func (eb *EdsmainBlb) Deleted() bool {
	return eb._deleted
}

// Insert inserts the EdsmainBlb to the database.
func (eb *EdsmainBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.edsmain_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the EdsmainBlb in the database.
func (eb *EdsmainBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.edsmain_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the EdsmainBlb to the database.
func (eb *EdsmainBlb) Save(db XODB) error {
	if eb.Exists() {
		return eb.Update(db)
	}
...
// Upsert performs an upsert for EdsmainBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (eb *EdsmainBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the EdsmainBlb from the database.
func (eb *EdsmainBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.edsmain_blb WHERE blb_lrn = $1`
