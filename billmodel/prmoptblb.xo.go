...
// Exists determines if the PrmoptBlb exists in the database.
func (pb *PrmoptBlb) Exists() bool {
	return pb._exists
}

// Deleted provides information if the PrmoptBlb has been deleted from the database.
func (pb *PrmoptBlb) Deleted() bool {
	return pb._deleted
}

// Insert inserts the PrmoptBlb to the database.
func (pb *PrmoptBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.prmopt_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the PrmoptBlb in the database.
func (pb *PrmoptBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.prmopt_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the PrmoptBlb to the database.
func (pb *PrmoptBlb) Save(db XODB) error {
	if pb.Exists() {
		return pb.Update(db)
	}
...
// Upsert performs an upsert for PrmoptBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (pb *PrmoptBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the PrmoptBlb from the database.
func (pb *PrmoptBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.prmopt_blb WHERE blb_lrn = $1`
