...
// Exists determines if the PprecBlb exists in the database.
func (pb *PprecBlb) Exists() bool {
	return pb._exists
}

// Deleted provides information if the PprecBlb has been deleted from the database.
func (pb *PprecBlb) Deleted() bool {
	return pb._deleted
}

// Insert inserts the PprecBlb to the database.
func (pb *PprecBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.pprec_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the PprecBlb in the database.
func (pb *PprecBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.pprec_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the PprecBlb to the database.
func (pb *PprecBlb) Save(db XODB) error {
	if pb.Exists() {
		return pb.Update(db)
	}
...
// Upsert performs an upsert for PprecBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (pb *PprecBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the PprecBlb from the database.
func (pb *PprecBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.pprec_blb WHERE blb_lrn = $1`
