...
// Exists determines if the CommsBlb exists in the database.
func (cb *CommsBlb) Exists() bool {
	return cb._exists
}

// Deleted provides information if the CommsBlb has been deleted from the database.
func (cb *CommsBlb) Deleted() bool {
	return cb._deleted
}

// Insert inserts the CommsBlb to the database.
func (cb *CommsBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.comms_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the CommsBlb in the database.
func (cb *CommsBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.comms_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the CommsBlb to the database.
func (cb *CommsBlb) Save(db XODB) error {
	if cb.Exists() {
		return cb.Update(db)
	}
...
// Upsert performs an upsert for CommsBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (cb *CommsBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the CommsBlb from the database.
func (cb *CommsBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.comms_blb WHERE blb_lrn = $1`
