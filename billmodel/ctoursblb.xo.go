...
// Exists determines if the CtoursBlb exists in the database.
func (cb *CtoursBlb) Exists() bool {
	return cb._exists
}

// Deleted provides information if the CtoursBlb has been deleted from the database.
func (cb *CtoursBlb) Deleted() bool {
	return cb._deleted
}

// Insert inserts the CtoursBlb to the database.
func (cb *CtoursBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.ctours_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the CtoursBlb in the database.
func (cb *CtoursBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.ctours_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the CtoursBlb to the database.
func (cb *CtoursBlb) Save(db XODB) error {
	if cb.Exists() {
		return cb.Update(db)
	}
...
// Upsert performs an upsert for CtoursBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (cb *CtoursBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the CtoursBlb from the database.
func (cb *CtoursBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.ctours_blb WHERE blb_lrn = $1`
