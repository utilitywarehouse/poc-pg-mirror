...
// Exists determines if the CaciBlb exists in the database.
func (cb *CaciBlb) Exists() bool {
	return cb._exists
}

// Deleted provides information if the CaciBlb has been deleted from the database.
func (cb *CaciBlb) Deleted() bool {
	return cb._deleted
}

// Insert inserts the CaciBlb to the database.
func (cb *CaciBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.caci_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the CaciBlb in the database.
func (cb *CaciBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.caci_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the CaciBlb to the database.
func (cb *CaciBlb) Save(db XODB) error {
	if cb.Exists() {
		return cb.Update(db)
	}
...
// Upsert performs an upsert for CaciBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (cb *CaciBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the CaciBlb from the database.
func (cb *CaciBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.caci_blb WHERE blb_lrn = $1`
