...
// Exists determines if the BiscontBlb exists in the database.
func (bb *BiscontBlb) Exists() bool {
	return bb._exists
}

// Deleted provides information if the BiscontBlb has been deleted from the database.
func (bb *BiscontBlb) Deleted() bool {
	return bb._deleted
}

// Insert inserts the BiscontBlb to the database.
func (bb *BiscontBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.biscont_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the BiscontBlb in the database.
func (bb *BiscontBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.biscont_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the BiscontBlb to the database.
func (bb *BiscontBlb) Save(db XODB) error {
	if bb.Exists() {
		return bb.Update(db)
	}
...
// Upsert performs an upsert for BiscontBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (bb *BiscontBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the BiscontBlb from the database.
func (bb *BiscontBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.biscont_blb WHERE blb_lrn = $1`
