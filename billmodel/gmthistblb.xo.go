...
// Exists determines if the GmthistBlb exists in the database.
func (gb *GmthistBlb) Exists() bool {
	return gb._exists
}

// Deleted provides information if the GmthistBlb has been deleted from the database.
func (gb *GmthistBlb) Deleted() bool {
	return gb._deleted
}

// Insert inserts the GmthistBlb to the database.
func (gb *GmthistBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.gmthist_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the GmthistBlb in the database.
func (gb *GmthistBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.gmthist_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the GmthistBlb to the database.
func (gb *GmthistBlb) Save(db XODB) error {
	if gb.Exists() {
		return gb.Update(db)
	}
...
// Upsert performs an upsert for GmthistBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (gb *GmthistBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the GmthistBlb from the database.
func (gb *GmthistBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.gmthist_blb WHERE blb_lrn = $1`
