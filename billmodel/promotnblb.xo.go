...
// Exists determines if the PromotnBlb exists in the database.
func (pb *PromotnBlb) Exists() bool {
	return pb._exists
}

// Deleted provides information if the PromotnBlb has been deleted from the database.
func (pb *PromotnBlb) Deleted() bool {
	return pb._deleted
}

// Insert inserts the PromotnBlb to the database.
func (pb *PromotnBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.promotn_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the PromotnBlb in the database.
func (pb *PromotnBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.promotn_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the PromotnBlb to the database.
func (pb *PromotnBlb) Save(db XODB) error {
	if pb.Exists() {
		return pb.Update(db)
	}
...
// Upsert performs an upsert for PromotnBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (pb *PromotnBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the PromotnBlb from the database.
func (pb *PromotnBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.promotn_blb WHERE blb_lrn = $1`
