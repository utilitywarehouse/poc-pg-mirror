...
// Exists determines if the GinvBlb exists in the database.
func (gb *GinvBlb) Exists() bool {
	return gb._exists
}

// Deleted provides information if the GinvBlb has been deleted from the database.
func (gb *GinvBlb) Deleted() bool {
	return gb._deleted
}

// Insert inserts the GinvBlb to the database.
func (gb *GinvBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.ginv_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the GinvBlb in the database.
func (gb *GinvBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.ginv_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the GinvBlb to the database.
func (gb *GinvBlb) Save(db XODB) error {
	if gb.Exists() {
		return gb.Update(db)
	}
...
// Upsert performs an upsert for GinvBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (gb *GinvBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the GinvBlb from the database.
func (gb *GinvBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.ginv_blb WHERE blb_lrn = $1`
