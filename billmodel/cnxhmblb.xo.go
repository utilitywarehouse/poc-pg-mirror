...
// Exists determines if the CnxhmBlb exists in the database.
func (cb *CnxhmBlb) Exists() bool {
	return cb._exists
}

// Deleted provides information if the CnxhmBlb has been deleted from the database.
func (cb *CnxhmBlb) Deleted() bool {
	return cb._deleted
}

// Insert inserts the CnxhmBlb to the database.
func (cb *CnxhmBlb) Insert(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `INSERT INTO equinox.cnxhm_blb (` +
		`blb_data, blb_text` +
		`) VALUES (` +
		`$1, $2` +
		`) RETURNING blb_lrn`
...
// Update updates the CnxhmBlb in the database.
func (cb *CnxhmBlb) Update(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `UPDATE equinox.cnxhm_blb SET (` +
		`blb_data, blb_text` +
		`) = ( ` +
		`$1, $2` +
		`) WHERE blb_lrn = $3`
...
// Save saves the CnxhmBlb to the database.
func (cb *CnxhmBlb) Save(db XODB) error {
	if cb.Exists() {
		return cb.Update(db)
	}
...
// Upsert performs an upsert for CnxhmBlb.
//
// NOTE: PostgreSQL 9.5+ only
func (cb *CnxhmBlb) Upsert(db XODB) error {
	var err error
...
// Delete deletes the CnxhmBlb from the database.
func (cb *CnxhmBlb) Delete(db XODB) error {
	var err error
	...
	// sql query
	const sqlstr = `DELETE FROM equinox.cnxhm_blb WHERE blb_lrn = $1`
